...
type UserDataQueryHandler struct {
	unitOfWork     contracts.IUnitOfWork
	userRepository contracts.IUserRepository
}

func NewUserDataQueryHandler(unitOfWork contracts.IUnitOfWork,
	userRepository contracts.IUserRepository) *UserDataQueryHandler {
	return &UserDataQueryHandler{
		unitOfWork:     unitOfWork,
		userRepository: userRepository}
}

func (handler UserDataQueryHandler) Handle(ctx context.Context) (*UserQueryResponse, error) {
	var response UserQueryResponse
	userId, ok := application.GetUserIdFromContext(ctx)
	if !ok {
		return nil, applicationErrors.NewApplicationError("UserDataQueryHandler.MissingUserContext",
	...
	if err != nil {
		return nil, applicationErrors.NewApplicationError("UserDataQueryHandler.ExecuteTransaction",
