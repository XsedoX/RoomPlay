...
const factory = (options?: Parameters<typeof mount>[1], piniaStubs?: boolean | string[] | ((actionName: string, store: StoreGeneric) => boolean)) =>
...
async function openRoomPopup(wrapper: ReturnType<typeof factory>) {
  ...
  const passwordField = createRoomPopup.get('[data-testid="create-room-popup-password-input"] input');
  const repeatPasswordField = createRoomPopup.get('[data-testid="create-room-popup-repeat-password-input"] input');
  return { createRoomPopup:createRoomPopup, nameField:nameField, passwordField:passwordField, repeatPasswordField:repeatPasswordField };
}
...
describe('Main Menu', () => {
  ...
  it("checks if the CreateRoomPopup calls the createRoom method with the correct parameters", async () => {
    ...
    const { createRoomPopup, nameField, passwordField, repeatPasswordField } = await openRoomPopup(wrapper);
  ...
  it("checks if 'Join a Room' popup renders correctly on mobile", async () => {
    ...
    const scanBtnDom = joinRoomPopup.findAll('button').find(b => b.text().includes('Scan a QR Code'));
    expect(scanBtnDom?.isVisible()).toBeTruthy()
    expect(joinRoomPopup.get('[data-testid="join-room-popup-name-input"]').isVisible()).toBeTruthy()
    expect(joinRoomPopup.get('[data-testid="join-room-popup-password-input"]').isVisible()).toBeTruthy()
  })
