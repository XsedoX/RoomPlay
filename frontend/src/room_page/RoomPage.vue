...
const exampleSongs: Ref<ISongListDto[]> = ref([
  {
...
<template>
  <v-container min-width="320px" fluid class="pa-0 pb-4 ma-0 h-screen d-flex flex-column">
    <v-row no-gutters>
      <v-col cols="12">
        <PageTitle
          :users-initial-letters="usersInitialLetters"
          title="Rooom Name dawdwadwawaawd"
          class="px-1"
        >
          <template v-slot:top-left-corner>
            <v-btn color="red" rounded="xl" to="/mainMenu" variant="text"> Leave </v-btn>
          </template>
        ...
        >
          <v-sheet color="transparent" class="d-flex w-0 flex-column flex-shrink-1 flex-grow-1">
            <v-sheet color="transparent" class="d-flex align-center overflow-hidden ga-1">
              <touchscreen-tooltip
                text="Song Titledwdwdwdwdwdwdwdwd"
                :open-on-hover="false"
                v-slot="{ tooltipProps }"
              >
                <div
            ...
            <v-sheet color="transparent" class="d-flex align-center justify-start ga-2">
              <div class="on-surface-variant">0:45</div>
              <v-progress-linear height="6" color="primary" model-value="20"></v-progress-linear>
              <div class="on-surface-variant">3:45</div>
            </v-sheet>
          </v-sheet>
